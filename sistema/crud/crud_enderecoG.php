...
if(isset($_REQUEST["id_endereco"])) {
    ...
    $consulta = "INSERT INTO endereco (telefone,logradouro,num,complemento,bairro,cidade,estado,ativo,cep, regiao ,id_usuario) VALUES ('".$_REQUEST["telefone"]."','".$_REQUEST["logradouro"]."','".$_REQUEST["num"]."','".$_REQUEST["complemento"]."','".$_REQUEST["bairro"]."','".$_REQUEST["cidade"]."','".$_REQUEST["estado"]."','".$ativo."', '".$_REQUEST["cep"]."' , '".$_REQUEST["regiao"]."','".$_REQUEST["id_usuario"]."')";
            # a variavel operação está atrelada as mensagem de sucesso que aparecem na cor verde após realizar as funões do crud
            # tanto as mensagem de sucesso quando as de erro estão entrelaçadas a está variavel
    $operacao = 1;
...
if(!$query) {
    header("Location: ../Cad_enderecoG.php?erro=" . $operacao); 
   
} else {
    header("Location: ../Cad_enderecoG.php?sucesso=" . $operacao);
